% Auto-generated/updated from country-level policy counterfactual outputs.
\clearpage
...
\footnotesize
\RaggedRight
Notes: The figures compare observed headline inflation with a counterfactual path without the simulated price-policy measures. Country-level counterfactuals aggregate all available simulated policy components using HICP item weights. Unit: year-on-year inflation rate, in percent. Sources: Eurostat HICP, Bruegel, IMF, national policy sources, authors' calculations.
\par
\end{minipage}
\clearpage
