...
\caption*{%Notes:\\ 
Source: Eurostat HICP EA20 aggregate, authors' calculations}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{figure}[H]
\centering
\caption{Consumption baskets by income quintiles in the euro area (EA20, in \% of total expenditures)}
...
\begin{figure}[H]
\centering
\caption{Short-run inflation inequality in the euro area (EA20) by income quintile (in \%)}
\label{fig:ea20_short_run_income}
\includegraphics[scale=0.6]{fig/fig_EA20_2020_2026_income_inflation_Q1_Q5.png}
\vspace{20pt} 
\captionsetup{font=footnotesize, format=hang, singlelinecheck=false, justification=justified}
\caption*{Note: This figure reports year-on-year inflation rates for the first and fifth income quintiles in the euro area (EA20), January 2020 to April 2026.\\
...
\begin{figure}[H]
\centering
\caption{Inflation by household categories in the euro area (EA20, in \%)}
\label{fig:age_rural}
...
\vspace{10pt} 
\captionsetup{font=footnotesize, format=hang, singlelinecheck=false, justification=justified}
\caption*{Note: This figure reports year-on-year inflation rates in the euro area (EA20) by
...
the euro area (EA20) by income quintile, showing price indices from January 2000 to
...
\begin{figure}[H]
\centering
\caption{Long-run inflation inequality in the euro area (EA20) by income quintile (index, 2000 = 100)}
\label{fig:ea20_long_run_income}
\includegraphics[scale=0.7]{fig/fig_EA20_2000_2026_income_price_index_Q1_Q5_base2000.png}
\vspace{20pt} 
\captionsetup{font=footnotesize, format=hang, singlelinecheck=false, justification=justified}
\caption*{Note: This figure shows monthly price indices for the first and fifth income quintiles in the euro area (EA20), January 2000 to April 2026. Indices are rebased to 2000 = 100.\\
Sources: Eurostat HICP, HBS, authors' calculations}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
...
the difference between groups, such as $(w_{Q1}-w_{Q5})'\Delta p$ for the
income gap.
...
BE & 16.8 & 0.8 & 2.2 & -1.4
 \\
EA20 & 11.6 & 0.6 & 1.1 & -0.4
 \\
\bottomrule
\end{tabular}
\par\vspace{0.35em}\footnotesize\noindent Sources: Eurostat HICP, HBS, FIGARO, authors' calculations.
\par\vspace{0.25em}\footnotesize\noindent Notes. Entries are cumulative percentage-point effects of observed energy-price increases between January 2020 and December 2024. The direct shocks are country-specific cumulative HICP changes mapped to FIGARO energy sectors: upstream oil and gas extraction (B), refined petroleum products (C19), and electricity/gas utilities (D35). The price response is computed as $\Delta p = (I - A^{\prime})^{-1}s$, with $A$ built from the latest FIGARO matrix used in the simulations. The direct effect is the component from the shocked energy sectors in the household consumption basket. Indirect is the Leontief propagation component. Average inflation total includes direct and indirect effects before rounding. Inflation gap = Q1 - Q5.
\end{table}
\vspace{0.6em}
...
EA20 level, the income-quintile gap is positive but remains small relative to
the aggregate price effect. The decomposition shows that direct household
exposure raises the Q1--Q5 gap, while indirect production-network exposure
partly offsets it.
...
euro area (EA20). The first quintile faces a systematically larger burden than the
...
\begin{figure}[H]
\centering
\caption{Inflation burden in the euro area (EA20) by income quintile (in \% of disposable income)}
...
\begin{figure}[H]
\centering
\caption{Euro area (EA20): country-weighted policy counterfactual and official HICP inflation (in \%)}
