...
cumulative inflation gap between the first and fifth income quintiles was 0.6 percentage point in the
euro area excluding Italy, as product contributions and country-specific dynamics partly offset one
...
0.9 percentage point, while the rural--city gap peaked at 1.3 points in 2022 and reversed temporarily
...
price caps lowered average cumulative inflation by 1.2 percentage points and reduced the
income-quintile inflation gap by 0.8 point in the 19-country aggregate.
...
\citep{charalampakis2022impact,kiss2024inflation,gautier2024effects,corsello2026inflation}. Central
...
euro area excluding Italy, opposing product contributions and country-specific dynamics reduced the
aggregate gap to 0.6 percentage point. The inflation burden, which scales inflation by
...
Because chained indices are not additive, product-level contributions do not mechanically sum to the
aggregate year-on-year inflation rate unless the contribution formula accounts for the chain-linking
structure.
...
into marked inflation inequality by income quintile in the euro-area aggregate excluding Italy. Inflation
...
structures and country-level price dynamics are combined across the remaining 19 countries.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[H]
\centering
\caption{Short-run inflation inequality in the euro area excluding Italy, by income quintile (in \%)}
\label{fig:ea20_short_run_income}
\includegraphics[scale=0.6]{fig/fig_EA20_2020_2026_income_inflation_Q1_Q5.png}
\vspace{6pt}
\captionsetup{font=footnotesize, format=hang, singlelinecheck=false, justification=justified}
\caption*{Note: This figure reports year-on-year inflation rates for the first and fifth income quintiles in the euro area excluding Italy, January 2020 to July 2026. All countries, including Spain, are computed at COICOP digit 3; the 19 country weights are renormalised after excluding Italy.\\
...
Figure~\ref{fig:age_rural} presents inflation rates by household category in the euro area excluding
Italy, segmented by age and residence area. Panel (a) shows that annual inflation among households
aged 60 or over rose above that of households under 30 during the energy-price surge; the difference
peaked at about 0.9 percentage point in August 2022 and then narrowed. Panel (b) likewise shows a
...
\begin{figure}[H]
\centering
\caption{Inflation by household categories in the euro area excluding Italy (in \%)}
\label{fig:age_rural}
...
\vspace{10pt}
\captionsetup{font=footnotesize, format=hang, singlelinecheck=false, justification=justified}
\caption*{Note: This figure reports year-on-year inflation rates in the euro area excluding Italy by
age (panel a) and residence area (panel b), January 2020 to July 2026. All country calculations use COICOP digit 3.\\
Sources: Eurostat HICP, HBS, authors' calculations}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
...
\begin{figure}[H]
\centering
\caption{Long-run inflation inequality in the euro area excluding Italy, by income quintile (index, 2000 = 100)}
...
\vspace{6pt}
\captionsetup{font=footnotesize, format=hang, singlelinecheck=false, justification=justified}
\caption*{Note: This figure shows monthly price indices for the first and fifth income quintiles in the euro area excluding Italy, January 2000 to July 2026. Indices are rebased to 2000 = 100; all countries use COICOP digit 3.\\
Sources: Eurostat HICP, HBS, authors' calculations}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
...
Table~\ref{tab:ea20-income-gap-product-contributions} decomposes the euro-area-excluding-Italy income
...
trade linkages, and household expenditure weights differ across countries. For the euro area excluding Italy,
...
\begin{figure}[H]
\centering
\caption{Inflation burden in the euro area excluding Italy, by income quintile (in \% of disposable income)}
...
We aggregate the country-level policy counterfactuals using Eurostat HICP country weights after
excluding Italy and renormalizing the remaining 19 countries. The same country scope and annual
...
inflation for the euro area excluding Italy and the corresponding no-policy counterfactual is largest
...
\begin{figure}[H]
\centering
\caption{Euro area excluding Italy: observed and no-policy inflation (in \%)}
...
\captionsetup{font=footnotesize, format=hang, singlelinecheck=false, justification=justified}
\caption*{Notes: The figure compares observed and no-policy inflation for 19 euro-area countries, excluding Italy and renormalizing annual HICP country weights, from January 2021 to July 2026. Unit: year-on-year inflation rate, in percent.\\
...
\begin{figure}[H]
\centering
\caption{No-policy short-run inflation inequality in the euro area excluding Italy, by income quintile (in \%)}
...
\captionsetup{font=footnotesize, format=hang, singlelinecheck=false, justification=justified}
\caption*{Notes: The figure reports counterfactual year-on-year inflation rates for the first and fifth income quintiles in the euro area excluding Italy, from January 2021 to July 2026. All countries, including Spain, use COICOP digit 3 and RAS expenditure weights; the remaining 19 annual HICP country weights are renormalized.\\
...
For the euro area excluding Italy, cumulative inflation from the total no-policy HICP index is 15.9 percent.
Counterfactual cumulative inflation is 16.8 percent for Quintile 1 and 15.3 percent for Quintile 5,
compared with observed values of 14.8 and 14.2 percent reported in Table~\ref{tab:country-heterogeneity-cumulative-gap}.
The observed Quintile 1--Quintile 5 gap is therefore 0.6 percentage point,
compared with 1.4 percentage points without the simulated measures. Under our sign convention,
the resulting policy effect of $-0.8$ percentage point means that fiscal policies lowered the
relative inflation of Quintile 1 in the 19-country aggregate.
...
excluding Italy, the cumulative Quintile 1--Quintile 5 gap was 0.6 percentage point. Differences by
...
While Italy reports HBS data annually on the national statistical office's website, the most recent
Eurostat harmonised and complete HBS entry for Italy is 2005. This requires a specific treatment for
some household-group data. We therefore construct estimated 2015 and 2020 age and income-quintile
baskets.

For age groups, the Italian Istat public-use HBS files for 2015 and 2020 are read directly from the
original files, using the ECOICOP v1 expenditure aggregates available in the microdata. These direct
division-level expenditure variables are used to build age-group HBS inputs by the age of the
household reference person.

For income quintiles, we first use the year in which income-quintile and expenditure-quintile
information can be compared to build a 2005 income/expenditure bridge by COICOP division and
quintile, and then apply this bridge to the target-year Istat HBS baskets by equivalised-expenditure
quintile before renormalising each corrected basket to 100 percent. These bridge-corrected
income-quintile baskets are then used as target-year calibration targets for the latent income-score
model, while the household-level 2015 and 2020 Istat HBS public-use files provide the
characteristics and expenditure patterns used to form the target-year groups.

For each household $i$ in target year $y$, let $X_{i,y}$ denote observed characteristics such as age
and education of the reference person, labour-market status, occupation, housing tenure, housing
surface, ownership of durable goods, internet access, subjective economic resources and situation,
the absolute-poverty indicator, and household consumption level. A latent socio-economic score is
defined as:
...
Let $r_{i,y}(\beta_y)$ be the weighted rank of household $i$ in the target-year distribution of this
score. The probability that household $i$ belongs to latent income group $g \in \{1,\ldots,5\}$ is:
...
where $m_g \in \{0.1,0.3,0.5,0.7,0.9\}$ is the centre of quintile $g$ and $\tau$ governs the
sharpness of the probabilistic assignment. This soft assignment avoids forcing households close to a
quintile boundary into a single group with probability one.

For COICOP division $j$, the probabilistic expenditure share of group $g$ in target year $y$ is:
...
where $w_{i,y}$ is the household survey weight and $x_{i,j,y}$ is household expenditure on product
division $j$. The score parameters are calibrated at the aggregate level by matching the target-year
probabilistic baskets to the bridge-corrected target-year income-quintile basket:
...
The resulting groups should therefore be interpreted as latent income groups rather than directly
observed income quintiles. The bridge allows the broad income-group structure to vary with the
target-year expenditure distribution, while retaining the 2005 evidence on how income-quintile and
expenditure-quintile baskets differ.

Table~\ref{tab:it-hbs-all-products} reports the aggregate all-products consumption shares used to
document the available Italian HBS inputs. For age groups, the entries are true survey-weighted
shares of annual household consumption in the target year, computed as group consumption divided by
total household consumption. For income groups, the 2015 and 2020 groups are latent rather than
directly observed survey groups; their aggregate shares are the bridge-corrected target-year
estimates. The resulting Italian distribution is close to the pattern observed in neighbouring large
euro-area countries: the first quintile accounts for about 8 percent of total consumption, while the
fifth quintile accounts for about 39 percent, compared with roughly 9--10 percent and 33--36 percent
in Germany, France and Spain. Table~\ref{tab:it-hbs-latent-income-coefficients} reports the
coefficients of the latent income-score model used to construct these target-year baskets.
