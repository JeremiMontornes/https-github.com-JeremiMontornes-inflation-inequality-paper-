...
Our research question is: how have the recent energy-price shocks and the fiscal measures adopted in
...
the country and the euro-area level. This framework also supports household-level analysis using
...
the rural--city ranking temporarily reversed in 2023. In household-level regressions for Belgium,
Germany, Spain, France, and the Netherlands, residence density is the most consistent correlate of
inflation exposure, whereas the associations with income rank and household size vary across years.
...
Sources: Eurostat HBS 2020; authors' calculations.}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\vspace{6pt}
\captionsetup{font=footnotesize, format=hang, singlelinecheck=false, justification=justified}
\caption*{Notes: This figure reports year-on-year inflation rates for the first and fifth income quintiles in the euro area, January 2020 to July 2026. All 20 countries, including Italy and Spain, are computed at COICOP digit 2 and aggregated using annual HICP country weights.\\
...
Sources: Eurostat HICP and country weights; Bruegel energy-policy dataset; IMF policy inventory; national policy sources; authors' calculations.}
\end{figure}
...
Sources: Eurostat HICP and country weights; household budget surveys; Bruegel energy-policy dataset; IMF policy inventory; national policy sources; authors' calculations.}
\end{figure}
...
and Germany and Spain ($-0.9$ each). Finland is the only country reported in the table with a small
positive estimate ($0.1$). Italy is included in the 20-country aggregate and has an estimated policy
effect of $-0.3$ percentage point. Estimates close to zero arise either where no policy
...
individual household records. We use the expenditure reported by each household in the HBS
microdata to recover a household-specific consumption basket, which we combine with monthly HICP
price indices. The unit of observation is therefore the household rather than an income, age, or
residence group. This approach reveals the dispersion hidden by group averages and allows us to
...
We next pool individual household records from Belgium, Germany, Spain, France, and the Netherlands
to examine whether differences in consumption baskets translate into systematic differences in
inflation exposure. Italy is not included in these regressions because sufficiently recent and
comparable household-level microdata are not available for the common specification; the
Italy-specific data used elsewhere in the paper do not provide the same set of individual-level
variables. The estimation sample therefore contains five countries, even though the broader
descriptive analysis covers a larger set of euro-area economies.

The dependent variable is the mean annual inflation rate implied by each household's own expenditure
shares. The regressors are equivalised-income-quintile indicators, household size, and the degree of
urbanisation of the household's area of residence. Table~\ref{tab:hbs-inflation-regressions-by-year}
...
\begin{table}[H]
\centering
\caption{Household-level inflation regressions by year, five-country sample}
\label{tab:hbs-inflation-regressions-by-year}
\scriptsize
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{0.9}
\input{tables/tab_BE_DE_ES_FR_IT_NL_hbs_inflation_regressions_by_year}
\vspace{4pt}
\captionsetup{font=footnotesize, format=hang, singlelinecheck=false, justification=justified}
\caption*{\emph{Notes:} The dependent variable is household-level mean annual inflation, in percent. Regressions are estimated separately by inflation year using household survey weights and include country fixed effects. The omitted categories are income quintile 1 and rural areas. The sample covers Belgium, Germany, Spain, France, and the Netherlands; Italy is excluded because comparable household-level variables are unavailable. Heteroskedasticity-robust standard errors are reported in parentheses.\\
\emph{Sources:} Household Budget Survey microdata; Eurostat HICP; authors' calculations.}
\end{table}

\begin{table}[H]
\centering
\caption{Pooled household-level inflation regression, five-country sample}
\label{tab:hbs-inflation-regressions-pooled}
\scriptsize
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{0.9}
\input{tables/tab_BE_DE_ES_FR_IT_NL_hbs_inflation_regression_pooled}
\vspace{4pt}
\captionsetup{font=footnotesize, format=hang, singlelinecheck=false, justification=justified}
\caption*{\emph{Notes:} The dependent variable is household-level mean annual inflation, in percent. The regression uses household survey weights and includes country and inflation-year fixed effects. The omitted categories are income quintile 1 and rural areas. The sample covers Belgium, Germany, Spain, France, and the Netherlands; Italy is excluded because comparable household-level variables are unavailable. Heteroskedasticity-robust standard errors are reported in parentheses.\\
\emph{Sources:} Household Budget Survey microdata; Eurostat HICP; authors' calculations.}
\end{table}

The income coefficients are small in economic terms but reveal that the gradient is neither stable
nor monotonic over time. Relative to the first quintile, households in the upper quintiles experienced
somewhat higher inflation in 2021: the difference rises from 0.16 percentage point for the second
quintile to 0.36 percentage point for the fifth. The gradient largely disappears in 2022, when the
shock was broad-based: the fifth-quintile coefficient is slightly negative and statistically
insignificant, while the other differences remain below 0.07 percentage point. In 2023, the third to
fifth quintiles again record slightly higher inflation than the first, but the estimated gaps do not
exceed 0.09 percentage point. In the pooled specification, all four income coefficients are positive
and statistically significant, ranging from 0.07 to 0.14 percentage point, with the largest estimate
for the fourth rather than the fifth quintile. The results therefore do not support a simple linear
relationship between income rank and household inflation over this period.

Household size is also associated with inflation exposure, but its sign varies across years. Each
additional household member is associated with 0.02 percentage point higher inflation in 2021,
0.08 percentage point lower inflation in 2022, and 0.14 percentage point higher inflation in 2023.
...
The most consistent pattern concerns location. Conditional on income, household size, and country
effects, households in intermediate-density and densely populated areas experience lower inflation
than households in rural areas in every year. The urban--rural gap is especially large in 2022:
inflation is estimated to be 0.67 percentage point lower in intermediate areas and 1.52 percentage
points lower in densely populated areas. The pooled estimates are respectively $-0.38$ and $-0.85$
percentage point. These gaps are consistent with rural households allocating larger shares to home
energy and private transport, the categories most exposed to the energy-price shock.
...
rural--city ordering temporarily reversed in 2023. In household-level regressions for Belgium,
Germany, Spain, France, and the Netherlands, residence density is consistently associated with
inflation exposure, whereas the income and household-size coefficients vary across years.
...
simulated measures lowered average cumulative inflation by 1.3 percentage points and reduced the
Quintile 1--Quintile 5 inflation gap by 1.1 percentage points.
