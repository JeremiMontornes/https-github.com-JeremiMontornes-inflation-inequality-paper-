...
\title{Inflation Inequalities Across the Euro Area\thanks{This paper presents the authors' views and should not be interpreted as reflecting the views of
 the Banque de France or the Eurosystem. This paper is based on HBS public and microdata from Eurostat. The responsibility for all conclusions drawn from the data lies entirely with the authors. We thank Pradana Aumars and Lyna El Kamel for excellent research assistance.}}
...
This paper examines the effect of the recent inflation surge on euro area households. Since mid-2021, inflation rates in the euro area have risen sharply, displaying considerable dispersion across countries and socio-demographic groups \citep{charalampakis2022impact,kiss2024inflation,gautier2024effects,corsello2026inflation}. Central banks target aggregate inflation, but the welfare cost of a given aggregate inflation rate is not necessarily uniform across households. High inflation disproportionately affects households whose consumption baskets are more exposed to rapidly rising prices, in particular households with higher expenditure shares on energy and food. The 2021--2023 episode was not only an aggregate inflation shock, but also a large relative-price shock, initially driven by imported energy prices. Its incidence therefore depended on household exposure to energy-intensive items, including heating and transport \citep{levell2024distributional}.
...
The paper is also related to policy work by \cite{bobasu2023impact}, \cite{claeys2022inflation}, \cite{gautier2024effects}, and \cite{kiss2024inflation}, who analyze inflation inequality at the euro area level.

A related strand of recent literature emphasizes the distinction between homothetic and non-homothetic preferences and the role of taste change in cost-of-living measurement \citep{redding2020measuring,martin2022revisiting}. In exact cost-of-living terms, proportional price changes leave relative prices unchanged and therefore do not create heterogeneous fixed-basket inflation rates. When price changes differ across products, by contrast, household inflation can be written as the interaction between budget shares and item-level price changes. Non-homothetic preferences provide one reason why these budget shares vary systematically with living standards, while age, household composition, housing status, and residence area generate additional variation in observed baskets. This connects exact cost-of-living index theory to evidence on quality adjustment in household demand \citep{coxWohlgenant1986prices,deaton1988quality} and to the recent literature on unequal gains from product innovation and cheapflation \citep{jaravel2019unequal,chen2024cheapflation,sangani2026cheapflation}. Our empirical approach is closer to the expenditure-share channel: we measure how HBS budget shares by income, age, and residence area interact with common item-level price changes, rather than estimating household-specific price changes within narrowly defined products.
...
The Household Budget Surveys (HBS) are conducted on a large sample of households in each euro area country. Households are required to document their expenditures on consumer goods and services over a designated period, typically two weeks. These surveys capture the values of expenditures across COICOP categories. HBS data are generally collected every five years, which creates challenges for measuring changes in consumption inequality between waves \citep{accardo2023measuring}. The most recent complete HBS wave generally refers to 2020. Data for this wave were collected between 2018 and 2022 in most EU Member States, with a few country-specific timing exceptions.\footnote{The following HBS wave is the 2025 wave, with increased harmonization between countries.} A few national statistical institutes also provide higher-frequency HBS data that can be used for country-specific extensions.

Repeated HBS waves available for a subset of countries suggest that, while behavioural changes over the course of a few years can be significant, percentage-point differences in the weights of the main consumption categories between low- and high-income households remain relatively stable. This relative stability is central to our estimates. Changes in overall consumption patterns may therefore have a limited effect on measured inflation inequality when consumption shares move in the same direction across income groups.
...
generally have greater capacity to absorb a price shock by reducing discretionary
...
For VAT or excise-tax reductions, the observed retail price includes the reduced policy tax rate. Following the standard tax-inclusive price logic used to assess the inflation effects of VAT changes \citep{gautier2013vat}, the no-policy price is obtained by backing out the reduced tax-inclusive price and reapplying the normal tax rate:
\begin{equation}
\widetilde p_{j,t}
=
p_{j,t}
...
Table~\ref{tab:country-heterogeneity-counterfactual-gap} extends
Table~\ref{tab:country-heterogeneity-cumulative-gap} by comparing observed
income-quintile inflation with a no-policy counterfactual. The first three
columns reproduce the cumulative inflation rates and Quintile 1--Quintile 5
gap reported in the earlier table. To construct the additional columns, we
replace the observed HICP paths of the affected COICOP components with the
country-specific counterfactual paths described above. We then aggregate these
component prices using the same income-quintile expenditure weights, annual
chaining procedure, 2020 base year, and euro-area country weights as in the
observed indices. The effect of unconventional fiscal policies is defined as
the observed inflation gap minus the counterfactual inflation gap. A negative
value therefore means that the measures lowered Quintile 1 inflation relative
to Quintile 5 inflation, while a positive value means that they raised it.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\input{tables/tab_EA20_country_income_q1_q5_counterfactual_gap_2020_2024}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

At the euro-area level, cumulative inflation is 19.8 percent for Quintile 1
and 20.6 percent for Quintile 5 in the observed data, compared with 20.8 and
21.5 percent in the no-policy counterfactual. The observed Quintile 1--Quintile
5 gap is therefore $-0.73$ percentage point, compared with $-0.62$ percentage
point without the simulated measures. Under our sign convention, the resulting
policy effect of $-0.11$ percentage point means that fiscal policies lowered
the relative inflation of Quintile 1 at the euro-area level.

The distributional effects are heterogeneous across countries. The largest
negative effects are estimated for Spain ($-0.87$ percentage point) and
Lithuania ($-0.38$), followed by Ireland, the Netherlands, and Germany. France
and Cyprus display small positive effects, while Italy has the largest positive
estimate ($0.23$ percentage point). Zero estimates arise either where no
policy layer from Appendix~\ref{sec:app-counterfactuals} enters the country row
or where temporary counterfactual price wedges leave no difference in the
2020--2024 endpoint comparison. Figure~\ref{fig:ea_policy_total_cpi_counterfactual}
and the monthly paths in Appendix~\ref{sec:app-counterfactuals} complement this
cumulative comparison by showing when the measures affected inflation.


\section{Household-level inflation}
\label{sec:household-level}

This section extends the analysis from broad household groups to individual households. It documents inflation dispersion within income, age, and residence categories and examines how observed household characteristics are associated with heterogeneous inflation exposure.

To measure this within-group heterogeneity, we combine household-level HBS expenditure data with monthly HICP price indices. In the empirical application, we use the Spanish EPF, a publicly available HBS microdata source, and match each annual price index to the preceding EPF wave. The HBS survey year is denoted $y'=y-1$ in the expenditure-share formulas below. Household-level inflation is obtained from the price index implied by each household's expenditure shares, and the resulting distributions are weighted using EPF survey weights.
...
effects. These regressions estimate the conditional association between each
characteristic and household-level inflation, holding the other observed
...
This paper develops timely inflation inequality indicators for the euro area using HICP prices and household budget surveys. The framework measures household-group inflation across countries and over time while preserving consistency with official aggregate inflation.

%results
Income-based inflation gaps remained contained during the recent inflation episode in several large economies, notably Germany, France, and Spain, but varied substantially across countries. Differences by age and residence were larger: older and rural households generally faced higher inflation because of their greater expenditure shares on food and energy. VAT reductions, fuel rebates, and energy price shields reduced average inflation and, in many cases, compressed inflation inequality.
...
Price-based fiscal measures can provide temporary distributional stabilization during large relative-price shocks, whereas monetary policy remains responsible for aggregate price stability. Income groups alone do not identify vulnerable households: exposure also depends on residence, heating technology, mobility constraints, and substitution possibilities.

%limit
The indicators remain subject to aggregation bias and exclude income and wealth channels. More granular scanner, transaction, and household expenditure data would improve measurement within broad consumption categories and household groups.
