...
\begin{abstract}
This article docuements inflation inequality across household categories in the euro area using
harmonized HICP prices and household budget surveys. We construct country-level inflation indicators
by income, age, and residence area, aggregate them to the euro area, and assess the role of recent
fiscal interventions through counterfactual price indices. During the 2021--2023 inflation episode,
income-quintile inflation gaps remained limited at the euro-area level because product contributions
often offset one another across countries. By contrast, differences by age and residence area were
more pronounced, with older and rural households more exposed to energy and essential goods while
Inflation burdens in tems of income, shpw stronger distributional effect on the first income
quintile. Counterfactual price indices show that tariff shields, VAT cuts, fuel rebates, and price
caps reduced average inflation and compressed inflation inequality during the energy crisis in the
euro area.
...
inflation rates in AIDS system. When price changes differ across products, by contrast, household
...
the country and th eeuro area level. Second, it examines the effect of unconventional fiscal
...
Our main results are the following. Inflation inequality exhibits substantial variation across euro
area countries. In several large economies, such as Germany, France, and Spain,  inflation gaps have
remained relatively low since 2021. In many cases, the inflation gap between the first and the fifth
income quintiles is close to zero, partly because the product contributions to the gap tend to
offset one another and because fiscal measures compressed some energy price increases. However, the
euro-area inflation burden, which scales inflation by consumption-to-income ratios, is substantially
larger for the first quintile than for the fifth quintile. More significant differences also emerge
when considering demographic factors such as age and geographical factors, particularly between
urban and rural households. Rural households face higher energy inflation in several countries,
reflecting greater reliance on private transport, longer travel distances, and fewer short-run
alternatives to fuel consumption.
...
relatively stable (see \footnote{Appendix~\ref{app:substitution-bias} Appendix C.5). Changes in
...
the income distribution, but also across demographic groups and territorial contexts. In line with
the Eurostat HICP methodology, imputed rent is excluded from the analysis. This analysis uses all
...
residence-area group. The HBS correction factor is defined as:
...
where $E^{HBS}_{j,g,y'}$ is HBS expenditure on product category $j$ by group $g$ in HBS wave $y'$,
and $E^{HBS}_{j,\mathrm{all},y'}$ is the corresponding expenditure for all households.
$E^{HBS}_{g,y'}$ and $E^{HBS}_{\mathrm{all},y'}$ denote total HBS consumption expenditure by group
$g$ and by all households, respectively. The coefficient $c^{HBS}_{j,g,y'}$ therefore measures the
share of aggregate HBS expenditure on product $j$ accounted for by group $g$.
...
into marked inflation inequality by income quintile at the euro-area aggregate level. Inflation
...
structures and country-level price dynamics are combined at the euro-area level.
...
segmented by age and residence area. Panel (a) shows inflation disparities by age, with older
households experiencing higher inflation rates because they spend more on essential goods. Panel (b)
highlights differences by residence area, with rural households facing higher inflation rates than
households living in cities.

The residence-area differentials are consistent with differences in the structure of energy demand.
Rural households tend to spend a larger share of their budget on transport fuels and home energy,
partly because commuting distances are longer and the set of available transport substitutes is more
limited. During an energy-price shock, these structural features translate into larger short-run
inflation exposure.
...
income quintile, showing price indices from January 2000 to April 2026. The long-run evolution of
...
with different price dynamics and fiscal responses that compensates eache other. Second,  xxx.
...
reporting the cumulative 2020--2024 inflation gap between the first and fifth income quintiles. The
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\input{tables/tab_EA20_country_income_q1_q5_cumulative_gap_2020_2024}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Table~\ref{tab:ea20-income-gap-product-contributions} decomposes the euro-area annual income
...
quintiles over the 2021--2024 shock period.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\input{tables/tab_EA20_income_gap_product_contributions_2020_2024}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


To separate direct household exposure from indirect production-network exposure, we complement the
descriptive evidence with an input-output price simulation calibrated to the cumulative energy shock
observed between 2020 and 2024. Country-specific cumulative HICP changes in energy components are
mapped to FIGARO energy sectors and propagated through the production network as $\Delta
...
\bottomrule
\end{tabular}
\par\vspace{0.25em}\footnotesize\noindent \emph{Notes:} Entries are cumulative percentage-point effects of observed energy-price increases between January 2020 and December 2024. The direct shocks are country-specific cumulative HICP changes mapped to FIGARO energy sectors: upstream oil and gas extraction (B), refined petroleum products (C19), and electricity/gas utilities (D35). The price response is computed as $\Delta p = (I - A^{\prime})^{-1}s$, with $A$ built from the latest FIGARO matrix used in the simulations. The direct effect is the component from the shocked energy sectors in the household consumption basket. Indirect is the Leontief propagation component. Average inflation total includes direct and indirect effects before rounding. Inflation gap = Quintile 1 - Quintile 5.
\par\vspace{0.25em}\footnotesize\noindent \emph{Sources:} Eurostat HICP; HBS; FIGARO; authors' calculations.
\end{table}
\vspace{0.6em}

Table~\ref{tab:oil-gas-shock} shows that indirect production-network effects account for most of the
simulated cumulative impact of the energy shock. The average effect is largest in Belgium and
France, reflecting the stronger cumulative increases in household energy components over 2020--2024.
At the At the euro-area level, the income-quintile gap is positive but remains small relative to the
aggregate price effect. The decomposition shows that direct household exposure raises the Quintile
1--Quintile 5 gap, while indirect production-network exposure partly offsets it. Additional
simulations for food-product price shocks and electronic-component price shocks are reported in
...
electricity, and fuel. While the latter are not explicitly targeted at lower-income households, they
...
Counterfactual price indices are constructed at the COICOP-3 digits level by replacing the observed
...
We aggregate the country-level policy counterfactuals to the euro-area level using Eurostat country
weights for the HICP. The aggregation is performed over the set of countries for which a
price-policy counterfactual is simulated, with country weights renormalized within this policy
sample in each year. This produces a common-scope comparison between the observed country-weighted
policy-sample index and the corresponding no-policy counterfactual. We also report the official
euro-area HICP index as an external benchmark for the observed aggregate series.

Figure~\ref{fig:ea_policy_total_cpi_counterfactual} shows that the simulated fiscal measures
compressed euro-area inflation during the energy crisis. The gap between the observed policy-sample
index and the counterfactual index is largest around the peak of the 2022--23 energy shock, when VAT
reductions, fuel rebates, price shields, regulated-price caps, and calibrated block-tariff layers
were simultaneously active across countries. The official euro-area HICP remains close to the
observed policy-sample aggregate, while not coinciding exactly because the policy sample excludes
countries without a simulated price-policy layer and because the aggregation is restricted to the
countries covered by the counterfactual exercise.
...
Table~\ref{tab:country-heterogeneity-counterfactual-gap} compares observed income-quintile inflation
with a no-policy counterfactual. We replace the observed HICP paths of the affected COICOP
components with the country-specific counterfactual paths described above. We then aggregate these
component prices using the same income-quintile expenditure weights, annual chaining procedure, 2020
base year, and euro-area country weights as in the observed indices. The effect of unconventional
fiscal policies is defined as the observed inflation gap minus the counterfactual inflation gap.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\input{tables/tab_EA20_country_income_q1_q5_counterfactual_gap_2020_2024}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

At the euro-area level, cumulative inflation is 19.8 percent for Quintile 1 and 20.6 percent for
Quintile 5 in the observed data, compared with 20.8 and 21.5 percent in the no-policy
counterfactual. The observed Quintile 1--Quintile 5 gap is therefore $-0.73$ percentage point,
compared with $-0.62$ percentage point without the simulated measures. Under our sign convention,
the resulting policy effect of $-0.11$ percentage point means that fiscal policies lowered the
relative inflation of Quintile 1 at the euro-area level.

The distributional effects are heterogeneous across countries. The largest negative effects are
estimated for Spain ($-0.87$ percentage point) and Lithuania ($-0.38$), followed by Ireland, the
Netherlands, and Germany. France and Cyprus display small positive effects, while Italy has the
largest positive estimate ($0.23$ percentage point). Zero estimates arise either where no policy
layer from Appendix~\ref{sec:app-counterfactuals} enters the country row or where temporary
counterfactual price wedges leave no difference in the 2020--2024 endpoint comparison.
...
%results
Income-based inflation gaps remained contained during the recent inflation episode in several large
economies, notably Germany, France, and Spain, but varied substantially across countries.
Differences by age and residence were larger: older and rural households generally faced higher
inflation because of their greater expenditure shares on food and energy. VAT reductions, fuel
rebates, and energy price shields reduced average inflation and, in many cases, compressed inflation
inequality.
...
structure in year $y$. The RAS seed is
\begin{equation}
Z_{j,g,y}
=
\max\left\{
...
Table~\ref{tab:de-normalization-validation} compares the two weights for Germany at COICOP level 2
...
Figure~\ref{fig:fr-validation-rate} compares level-2 RAS recalculated French inflation with
...
at COICOP level 3 with the official INSEE income decile series.
