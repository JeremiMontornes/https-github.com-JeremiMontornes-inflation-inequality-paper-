...
It is also related to policy work by \cite{bobasu2023impact} and \cite{claeys2022inflation}, who analyze inflation inequality at the euro area level.
...
This analysis is based on publicly available data, primarily the Harmonised Index of Consumer Prices (HICP) constructed by national statistical institutes in collaboration with Eurostat. The HICP offers a standardized measure of inflation, ensuring comparability among member states. Eurostat aggregates national HICPs. Each country's HICP is based on price changes for a fixed basket of goods and services, weighted according to household expenditure from country-specific sources, including national accounts, household budget surveys, and other national sources. The national indices are then aggregated into the European HICP using country-specific weights that reflect their share of total household consumption expenditure. The calculation ensures comparability by adhering to Regulation EU 2016/796 and consistent data collection practices. The HICP is compiled according to the classification of household expenditures, ECOICOP ver. 2.
...
The Household Budget Surveys (HBS) are conducted on a large sample of households in each euro area country. Households are required to document their expenditures on consumer goods and services over a designated period, typically two weeks. These surveys capture the values of expenditures across COICOP categories. HBS data are generally collected every five years. The most recent and complete HBS wave generally took place in 2020. The 2020 HBS wave was actually conducted between 2018 and 2022 in most EU Member States, while data for Cyprus, Malta, and France were gathered earlier, between 2015 and 2017.\footnote{The following HBS wave is the 2025 wave, with increased harmonization between countries.} A few countries, such as Italy and Spain, have yearly HBS surveys. 
...
table when available and from the reconstructed Istat inputs otherwise.\\
...
Ireland display large positive Q1--Q5 gaps, while several large economies show
...
\begin{table}[H]
\centering
\caption{Effect of a 20.0 Percent Shock to Crude Oil and Gas Prices, 2022 (effects in pp)}
...
Country & \shortstack{Average inflation\\Total} & \shortstack{Average inflation\\Direct} & \shortstack{Income inflation\\gap} & \shortstack{Direct income\\inflation gap} & \shortstack{Indirect income\\inflation gap}
 \\
\midrule
DE & 1.6 & 0.3 & -0.1 & 0.0 & -0.1
 \\
FR & 1.6 & 0.0 & -0.1 & 0.0 & -0.1
 \\
IT & 2.3 & 0.6 & -0.1 & -0.1 & -0.1
 \\
ES & 2.0 & 0.0 & 0.2 & 0.0 & 0.2
 \\
NL & 2.5 & 0.6 & -0.4 & -0.0 & -0.4
 \\
BE & 2.3 & 0.0 & -0.6 & 0.0 & -0.6
 \\
EA20 & 1.9 & 0.2 & -0.1 & -0.0 & -0.1
 \\
\bottomrule
\end{tabular}
\par\vspace{0.35em}\footnotesize\noindent Sources: Eurostat HICP, HBS, FIGARO, authors' calculations.
\par\vspace{0.25em}\footnotesize\noindent Notes. Entries are percentage-point effects of a 20.0 percent sectoral input price shock. The shock size is the increase in the NYMEX natural gas futures price (NG=F), from 3.73 on December 31, 2021 to 4.475 on December 30, 2022, the last available trading day before December 31, 2022. The price response is computed as $\Delta p = (I - A^{\prime})^{-1}s$, with $A$ built from FIGARO. The direct effect is the component from the shocked sector in the household consumption basket. Indirect is the Leontief propagation component. Average inflation total includes direct and indirect effects before rounding. Income inflation gap = Q1 - Q5.
\end{table}
\vspace{0.6em}
...
gaps remain small relative to the aggregate price effect. The decomposition of
the income gap shows that direct household exposure and indirect
production-network exposure partly offset each other in several countries.
Additional simulations for food-products price
