...
France & Gas tariff freeze and subsequent gas price cap; Regulated electricity tariff cap and TICFE/TCCFE reduction; Transport-fuel rebate of 18 then 30 cents per litre & CP0451, CP0452, CP0722 & 2021-11-01--2023-12-01 & 3 of 3 measures use monthly reference paths; the remainder use statutory inputs. \\
...
\end{longtable}
\par\vspace{0.35em}\footnotesize\noindent Notes: The table lists all 94 rows marked \texttt{include = TRUE} in \texttt{counterfactual\_measure\_assumptions.xlsx}. The simulation program assigns each measure to a tax, subsidy/rebate, or administered-price family and calibrates one measure-specific intensity parameter so that the geometric mean of its monthly implied price ratios matches the audited ratio reported here. Monthly external reference paths are used when available; otherwise the audit output identifies the aggregate-equivalent calibration. A calibrated input is an auditable aggregate-equivalent price factor when a complete statutory monthly no-policy tariff is unavailable. Source: authors' assumptions workbook and calculations.
\end{landscape}
